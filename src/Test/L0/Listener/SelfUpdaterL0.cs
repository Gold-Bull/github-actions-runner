        ...
        public async void TestSelfUpdateAsync_CloneHash_RuntimeAndExternals()
        {
            try
            {
                ...
                using (var hc = new TestHostContext(this))
                {
                    ...
                    _runnerServer.Setup(x => x.GetPackageAsync("agent", BuildConstants.RunnerPackage.PackageName, "3.999.0", true, It.IsAny<CancellationToken>()))
                         .Returns(Task.FromResult(new PackageMetadata() { Platform = BuildConstants.RunnerPackage.PackageName, Version = new PackageVersion("3.999.0"), DownloadUrl = _packageUrl, TrimmedPackages = new List<TrimmedPackageMetadata>() { new TrimmedPackageMetadata() } }));
        ...
        public async void TestSelfUpdateAsync_UseExternalsTrimmedPackage()
        {
            try
            {
                ...
                using (var hc = new TestHostContext(this))
                {
                    ...
                    _runnerServer.Setup(x => x.GetPackageAsync("agent", BuildConstants.RunnerPackage.PackageName, "3.999.0", true, It.IsAny<CancellationToken>()))
                         .Returns(Task.FromResult(new PackageMetadata() { Platform = BuildConstants.RunnerPackage.PackageName, Version = new PackageVersion("3.999.0"), DownloadUrl = _packageUrl, TrimmedPackages = trim }));
